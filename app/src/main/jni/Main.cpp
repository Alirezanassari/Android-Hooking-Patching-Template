...
void Player(void *instance) {
    ...
    if (instance != NULL) {
        if (Gold) {
            *(int *) ((uint64_t) instance + 0x38) = 999999;
        }
        if (Gem) {
            *(int *) ((uint64_t) instance + 0x34) = 999999;
            }
    }
...
void *hack_thread(void *) {
    ...
    do {
        sleep(1);
    } while (!isLibraryLoaded("libAlireza.so"));
...
void Init(JNIEnv *env, jclass clazz, jobject context) {
    ...
    Toast(env, context, OBFUSCATE("Modded by AlirezaNassari"), ToastLength::LENGTH_LONG);
...
int RegisterMain(JNIEnv *env) {
    ...
    jclass clazz = env->FindClass(OBFUSCATE("com/Alireza/Main"));
    if (!clazz)
        return JNI_ERR;
